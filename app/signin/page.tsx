...
const page =  () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    await postNewUserToTableUsers(userName, password);
  ...
  return (
          ...
          <form action="" onSubmit={handleSubmit} className="absolute text-white z-50 my-0 mx-auto   gap-4   top-[10%]  left-[40%] bg-black/80  w-[26rem] h-[75%] ">
        ...
        <input
          ...
          onChange={(e:ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)}
            ...
            type={showPassword ? 'password' : 'text'} className="bg-transparent h-12  w-[60%]   focus:placeholder:-translate-y-3  placeholder:absolute placeholder:z-50   placeholder:pb-0  focus:placeholder:text-xs   outline-0 ring-0    border-b-2   border-x-0 border-t-0  mt-10  ml-20" placeholder="Password " />
                  <button className="absolute right-28  top-[12rem]   transition   " onClick={() => setShowPassword(!showPassword)}> {
